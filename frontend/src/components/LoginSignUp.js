...
function LoginSignUp() {
  ...
  const doLogin = async (event) => {
    ...
    try {
      ...
      if (res.message == "Invalid Email or Password") {
        ...
        res.message ==
        "An email has been sent to your account to verify. You must verify before logging in."
      ) {
        setMessage(
          "An email has been sent to your account to verify. You must verify before logging in."
        );
  ...
  const doSignup = async (event) => {
    ...
    try {
      ...
      if (
        res.message ===
        "Created account successfully! An Email has been sent to your account, please verify to login."
      ) {
        setResultSignUp(
          "Created account successfully! An Email has been sent to your account, please verify to login."
        );
  ...
  const doRegisterVerify = async (event) => {
    try {
      const response = await fetch(
        "/api/registerVerification/:id/verify/:token",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        }
    ...
    >
      <Row style={{ zIndex: 1 }}>
        <Col
          ...
          style={{ padding: 0, margin: 0, maxWidth: "50%" }}
        >
        ...
        >
          <form onSubmit={doLogin}>
            ...
            />
            <p onClick={togglePanel} className="mt-3">
        ...
        <div className="overlay">
          <div
            className={`overlay-panel ${
              isSignUp ? "overlay-left" : "overlay-right"
            }`}
          >
            {isSignUp ? (
              <>
